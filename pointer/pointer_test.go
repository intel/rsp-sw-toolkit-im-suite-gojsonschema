...
func init() {
	if err := json.Unmarshal([]byte(TEST_DOCUMENT_STRING), &testDocumentJson); err != nil {
		return
	}
}
...
func BenchmarkParse(b *testing.B) {
	for i := 0; i < b.N; i++ {
		_, err := NewJsonPointer(`/definitions/simple/0/next`)
		if err != nil {
			b.Errorf("NewJsonPointer error %v", err.Error())
		}
	}
}
...
func BenchmarkParseWithEscape(b *testing.B) {
	for i := 0; i < b.N; i++ {
		_, err := NewJsonPointer(`/definiti~0ons/simple/0/next`)
		if err != nil {
			b.Errorf("NewJsonPointer error %v", err.Error())
		}
	}
...
func BenchmarkGet(b *testing.B) {
	p, err := NewJsonPointer(`/obj/d/1/f`)
	if err != nil {
		b.Errorf("NewJsonPointer error %v", err.Error())
	}
	b.ResetTimer()
...
func TestSetNode(t *testing.T) {
	...
	if err := json.Unmarshal([]byte(jsonText), &jsonDocument); err != nil {
